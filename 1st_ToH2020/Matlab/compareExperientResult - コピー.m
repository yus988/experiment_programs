...
for cd_times = 1:6
    ...
    if cd_times == 1 
        ...
        V2G = 0.800; %MMA7361L 1.5Gモード
        text(40,annotationTextTopPos+ PosOffset,'Blue = Vp2','Color',circleLineColor,'FontSize', annotationTextFontSize);
        text(40,50,'20Hz-2W','Color','black','FontSize',20);
    elseif cd_times == 2
        cd 'Hapbeat-20Hz-2W';
        V2G = 0.206; %MMA7361L 6Gモード
        circleLineColor = '#FF0000'; %赤
        text(40,annotationTextTopPos + 2*PosOffset,'Red = Hapbeat','Color',circleLineColor,'FontSize', annotationTextFontSize);
    elseif cd_times == 3
        cd 'vp2-80Hz-1W';
        figure;
        imshow(Underlayer_img);
...
% %------------
   ...
   %注釈
    ...
    viscircles(centerOfAnnotation, radiulOfCircles,'Color',circleLineColor,'LineStyle','-','LineWidth',2);
    % 円の大きさの注釈（重力加速度：円直径）
    if cd_times == 1 || cd_times == 4
        % 前面 or 側面
        viscircles([150  400],1 * radius_coef, 'Color','black');
        text(130,400,'1G','Color','black','FontSize',24);       
        viscircles([150  520],2 * radius_coef, 'Color','black');
        text(130,520,'2G','Color','black','FontSize',24);       
        viscircles([150  700],3 * radius_coef, 'Color','black');
        text(130,700,'3G','Color','black','FontSize',24);
        
%         % 背面
%         viscircles([100  250], 1 * radius_coef, 'Color','white');
%         text(80,250,'1G','Color','white','FontSize',16);       
%         viscircles([300 250], 2 * radius_coef, 'Color','white');
%         text(270,250,'2G','Color','white','FontSize',24);       
%         viscircles([500  250], 3 * radius_coef, 'Color','white');
%         text(470,250,'3G','Color','white','FontSize',24);
