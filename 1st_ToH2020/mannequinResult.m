...
%         V2G6 = 0.206 /9.80665; %MMA7361L 6Gモード = v/g v / (g*9.80665)
        V2G6 = 0.100 /9.80665; %ADXL354C  8Gモード = v/g v / (g*9.80665)
        nharm = 6;%thdの高調波数
        ...
        for i = 1:numFiles1e4
            ...
            Mx{k,4} =  rms(Mx{k,2}(:,1)) + rms(Mx{k,2}(:,2))+rms(Mx{k,2}(:,3));%3軸RMS値
                Mx{k,4} =sqrt(rms(Mx{k,2}(:,1))^2 + rms(Mx{k,2}(:,2))^2 + rms(Mx{k,2}(:,3))); % こうあるべき
...
%         'Marker',marker,'MarkerSize',4,'MarkerFaceColor',lineColor);
    plot(tmp(:,1),tmp(:,2),'Color',lineColor,'LineStyle',lineStyle, ...
        'Marker',marker,'MarkerSize',4,'MarkerFaceColor',lineColor);
    hold on
end
