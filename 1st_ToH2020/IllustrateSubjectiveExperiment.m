...
num_pointArray =  length(imgCell{1,1}) - 1 ; %緑マーカの列の数。imgフォルダの画像数から算出
...
for m = 1:size(c_Annotation,1)
    for k = 1: size(c_Annotation{m,1},1)
        ...
        % loop化
        Annotation(k+i, 3) = Mx{1,1}(k+i,1) * radius_coef; % 半径となる値を代入
    end
...
y_base = 20;
y_offset = 30;
text(x_base,y_base,'20Hz','Color','white','FontSize',20);        
