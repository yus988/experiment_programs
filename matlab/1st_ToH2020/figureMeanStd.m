...
for i = 1:size(RMS_Cell,1) % 周波数ごとのイテレート。Mxのcell参照
    for k =1: size(RMS_Cell{1,1},1) % 測定箇所ごとのイテレート。
        % matlabの関数を使うため、tmp行列に格納
        arrMeanStd(row,1) =  Mean_Cell{i,1}(k,4);
        arrMeanStd(row,2) =  Std_Cell{i,1}(k,4);
        row = row + 1;
    end
end
...
% viscircles([base_x  base_y + pos_offset1], 2 * radius_coef, 'Color',ecolor,'EnhanceVisibility',false,'LineWidth',lineWidth);
% viscircles([base_x  base_y + pos_offset2], 3 * radius_coef, 'Color',ecolor,'EnhanceVisibility',false,'LineWidth',lineWidth);
% saveas(gcf,strcat('refCircle','.png'));
% close;
% %%
...
if isHapbeat
    ...
    for i = 1:3
        ...
        elseif i == 2 %80Hz
            ...
            posY = txtPosY + posOffset;
            f = 4;
        elseif i == 3 %140Hz
            circleLineColor = '#FF00FF'; %マゼンタ
            posY = txtPosY + 2 * posOffset;
            f = 5;
        end
        %         text(txtPosX,posY,signalType,'Color',circleLineColor,'FontSize', annotationTextFontSize);
    ...
    saveas(gcf,strcat('1freqMeanDiff','.png'));
    close
    ...
    saveas(gcf,strcat('2freqStdDiff','.png'));
    close
    ...
    saveas(gcf,strcat('3freqCvDiff','.png'));
    close
end
