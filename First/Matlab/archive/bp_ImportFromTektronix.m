...
for i = 1:numFiles
    ...
    if  IsNXYZ  % 第1chが加速度の場合（acc.txtがフォルダにあるとき）2,3chと同様1chにも加速度用の処理を行う
        Mx{i,2}(:,4) = ( Mx{i,1}(:,4) - mean(Mx{i,1}(:,4)) ) / V2G6;
    else % 1chが張力（acc.txtが無いとき）の場合
    ...
    if IsNXYZ
        for k = 0:3
            ...
            % グラフx軸について
%                 if contains(pwd, "RVS")
%                      RMS_Graph40Hz(index40Hz,1) = ( (numFiles / 2 - (index40Hz + 1)) * 10); 
%                 else
%                     RMS_Graph40Hz(index40Hz,1) = ( (index40Hz + 1) * 10); % 入力電圧（最終的にはモータ両端電圧になるので、任意の行列が入ることになる）
%                 end
                AVG40Hz(index40Hz,1) = ( (index40Hz + 1) * 10); % 入力電圧（最終的にはモータ両端電圧になるので、任意の行列が入ることになる）
                AVG40Hz(index40Hz,k+2) = rms((Mx{i,2}(:,1+k))); % グラフ描画用行列にrms値を都度代入
                AVG40Hz(index40Hz,6) = xyz;
            else
                index160Hz = i - numFiles / 2;
                AVG160Hz(index160Hz,1) = ( (index160Hz + 1) * 10); % 入力電圧（最終的にはモータ両端電圧になるので、任意の行列が入ることになる）
                AVG160Hz(index160Hz,k+2) = rms((Mx{i,2}(:,1+k))); % グラフ描画用行列にrms値を都度代入
                AVG160Hz(index160Hz,6) = xyz;
            end
            Mx{i,8+3*k} = thd_db;
        end
